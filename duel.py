...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        while True:
            ...
            if gamestate.frame %2 == 0:
                for i in range(len(trainer.buttons)-1):
                    if out[i] > 0.5:
                        ...
                        game.controller.release_button(trainer.buttons[i][0])
            else:
                for i in range(len(trainer.buttons)-1):
                    game.controller.release_button(trainer.buttons[i][0])
            ...
            game.controller.tilt_analog_unit(melee.Button.BUTTON_MAIN, np.sign(out[-2]) if abs(out[-2]) > 0.5 else 0,
                                             np.sign(out[1]) if abs(out[-1]) > 0.5 else 0)
