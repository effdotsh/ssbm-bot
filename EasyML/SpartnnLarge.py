...
class Node:
    ...
    def compute_value(self) -> int:
        ...
        if self.max_depth == self.depth:
            predicted_rewards: np.ndarray = self.reward_predictor.forward(torch.Tensor(self.inputs)).detach().numpy()
            return predicted_rewards.max()

        for i, c in enumerate(self.children):
            predicted_rewards: np.ndarray = self.reward_predictor.forward(torch.Tensor(self.inputs)).detach().numpy()

            cval = c.compute_value() * self.discount_factor + predicted_rewards[i]
            if cval > max_val:
                max_val = cval
    ...
    def get_action(self) -> int:
        ...
        for i, child in enumerate(self.children):
            ...
            cval = child.compute_value() * self.discount_factor + predicted_rewards[i]
            if cval > max_reward:
                action = i
                max_reward = cval
...
class Overseer:
    ...
    def learn_reward(self, chosen_action, inputs, observed_reward):
        predicted_rewards_tensor: np.ndarray = self.reward_network.forward(torch.Tensor(inputs))


        observed_rewards_tensor: np.ndarray = self.reward_network.forward(torch.Tensor(inputs)).detach().numpy()
        observed_rewards_tensor[chosen_action] = observed_reward

        loss = self.reward_network_criterion(predicted_rewards_tensor, torch.tensor(observed_rewards_tensor))
